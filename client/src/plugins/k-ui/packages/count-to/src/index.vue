<template>
  <div class="k-count-to">
    ...
    <div class="k-count-to__outer">
      <span :class="['k-count-to__text', countClass]" :id="counterId">{{ init }}</span>
...
export default defineComponent({
  ...
  setup(props) {
    const counterId = `count_to_${+new Date()}${Math.random()}`
    ...
    const getHandleVal = (val: number, len: number) => {
      return {
        endVal: parseInt(String(val / 10 ** props.unit[len - 1][0])),
    ...
    const transformValue = (val: number) => {
      ...
      if (val < 10 ** props.unit[0][0]) res.endVal = val
      else {
        for (let i = 1; i < len; i++) {
          if (val >= 10 ** props.unit[i - 1][0] && val < 10 ** props.unit[i][0]) res = getHandleVal(val, i)
        }
      }
      if (val > 10 ** props.unit[len - 1][0]) res = getHandleVal(val, len)
      return res
    }
    ...
    onMounted(() => {
      ...
      counter.value = new CountUp(counterId, endVal, {
